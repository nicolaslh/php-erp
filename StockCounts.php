...
echo '<form action="StockCounts.php?Action=Enter" method="post" enctype="multipart/form-data">';
echo '<div class="centre">';

echo '<p class="page_title_text"><img src="' . $RootPath . '/css/' . $Theme . '/images/inventory.png" title="' .
    _('Inventory Adjustment') . '" alt="" />' . ' ' . $Title . '</p>';

echo '<a href="/stocktaking.csv">下载模板</a><br>';
        ...
        if (isset($_POST['EnterByCat'])) {
            ...
            if (isset($_FILES['userfile']) and $_FILES['userfile']['name']) { //start file processing
                ...
                while (($myrow = fgetcsv($FileHandle, 10000, ",")) !== FALSE) {
                    $barcode = $myrow[0]; //条形码
                    $stockID = iconv("GBK", "UTF-8", $myrow[1]); //物料号
                    $qty = $myrow[2];       //数量
                    $ref = iconv("GBK", "UTF-8", $myrow[3]);   //摘要
                    if(!is_numeric($qty)){
                        prnMsg("数量必须为数字");
                    }

                    echo '<tr>
						<td><input type="text" name="BarCode_' . $row . '" value="'.$barcode.'" maxlength="20" size="20" /></td>
						<td><input type="text" name="StockID_' . $row . '" value="'.$stockID.'" maxlength="20" size="20" /></td>
						<td><input type="text" name="Qty_' . $row . '" value="'.$qty.'" maxlength="10" size="10"/></td>
						<td><input type="text" name="Ref_' . $row . '" value="'.$ref.'" maxlength="20" size="20"/></td>
					</tr>';
                ...
                for ($RowCount = 1; $RowCount <= 10; $RowCount++) {
                    echo '<tr>
						<td><input type="text" name="BarCode_' . ($RowCount + $row) . '" maxlength="20" size="20" /></td>
						<td><input type="text" name="StockID_' . ($RowCount + $row) . '" maxlength="20" size="20" /></td>
						<td><input type="text" name="Qty_' . ($RowCount + $row) . '" maxlength="10" size="10" /></td>
						<td><input type="text" name="Ref_' . ($RowCount + $row) . '" maxlength="20" size="20" /></td>
					</tr>';
